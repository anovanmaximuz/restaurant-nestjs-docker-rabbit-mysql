...
export class AppController {
  ...
  async create(@Body() createOrderDto: CreateOrderDto, @Res() response: Response) {
      ...
      let userExist = await this.appService.findUser(createOrderDto.user_id);
      if(userExist==0) { 
        throw new HttpException("We did not find any user data", HttpStatus.BAD_REQUEST);      
      }
      //check menu exist
      let isMenuExist =  await this.appService.findFood(createOrderDto.food_id);
      if(isMenuExist==0) { 
        throw new HttpException("No food menu available, please see the existing menu list!", HttpStatus.BAD_REQUEST);      
      }
    
      let hasFinishOrder =  await this.appService.hasFinishOrder(createOrderDto.order_id);
      if(hasFinishOrder>0){
        throw new HttpException("Order ID:"+createOrderDto.order_id+" being processed", HttpStatus.BAD_REQUEST);      
      }

      let hasPendingOrder = await this.appService.hasPendingOrder(createOrderDto.order_id);  
  
      if(hasPendingOrder>0){        
        let getPendingOrder = await this.appService.getPendingOrder(createOrderDto.order_id);
        let order_id = getPendingOrder.order_id;
        createOrderDto.order_id = order_id;
        let insert = await this.appService.create(createOrderDto);                
  ...
  async checkout(@Body() createCheckoutDto: CreateCheckoutDto, @Res() response: Response) {
    ...
    let hasPendingOrder = await this.appService.hasPendingOrder(createCheckoutDto.order_id);
    if(hasPendingOrder==0){    
      throw new HttpException("You have to order first", HttpStatus.BAD_REQUEST); 
    }

    let getPendingOrder = await this.appService.getPendingOrder(createCheckoutDto.order_id);
    let userInfo = await this.appService.getUser(getPendingOrder.user_id);
    
    //order_id,name,email
    let msgFormat = getPendingOrder.order_id+"|"+userInfo.name+"|"+userInfo.email;
    this.appService.fanOutPub(["order_confirmation","order_notification"], msgFormat);
    return response.status(HttpStatus.OK).send({
      statusCode: HttpStatus.OK,
  ...
  async getAllMenu(@Res() response: Response) {
    let menu = await this.appService.findAll();
  ...
  async findFood(@Param('id') id: number, @Res() response: Response) {
    let food = await this.appService.getFood(+id);
    if(food != null){
      return response.status(HttpStatus.OK).send({
        statusCode: HttpStatus.OK,
        message: 'success',
  ...
  async findAllOrder(@Param('id') id: string, @Res() response: Response) {
    let orders = await this.appService.findAllOrder(+id);
  ...
  async findOrderStatus(@Param('id') id: string,  @Res() response: Response) {
    let statusOrder = await this.appService.findOrderStatus(+id);
    if(statusOrder != null){
      return response.status(HttpStatus.OK).send({
        statusCode: HttpStatus.OK,
        message: 'success',
 ...
 // }
  ...
  async remove(@Param('id') id: string, @Res() response: Response) {
    let deleted = await this.appService.remove(+id);
    if(deleted.count > 0){
      return response.status(HttpStatus.OK).send({
        statusCode: HttpStatus.OK,
